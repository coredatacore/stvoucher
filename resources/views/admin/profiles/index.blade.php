...
@section('header', 'Voucher Profiles')
        ...
        <table class="table table-hover datatable align-middle mb-0 w-100">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-success fw-bold">₱{{ number_format($profile->price, 2) }}</td>
                    <td><span class="badge bg-light text-dark border"><i class="bi bi-clock me-1"></i>{{ $profile->duration }} {{ $profile->duration_unit }}</span></td>
                    <td>
                        <span class="badge rounded-pill {{ $profile->status == 'active' ? 'bg-success bg-opacity-10 text-success' : 'bg-danger bg-opacity-10 text-danger' }} px-3 py-2">
                            <i class="bi {{ $profile->status == 'active' ? 'bi-check-circle' : 'bi-x-circle' }} me-1"></i>{{ ucfirst($profile->status) }}
                        </span>
                    </td>
                    <td class="text-end">
